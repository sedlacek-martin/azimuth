...
$(function () {
    ...
    if (phoneInput !== null && phoneInput !== undefined) {
        let iti = window.intlTelInput(phoneInput, {
            // phone input only shows european countries
            onlyCountries: ["al", "ad", "at", "by", "be", "ba", "bg", "hr", "cz", "dk",
                "ee", "fo", "fi", "fr", "de", "gi", "gr", "va", "hu", "is", "ie", "it", "lv",
                "li", "lt", "lu", "mk", "mt", "md", "mc", "me", "nl", "no", "pl", "pt", "ro",
                "ru", "sm", "rs", "sk", "si", "es", "se", "ch", "ua", "gb"],
            geoIpLookup: function (callback) {
                $.get('//ipinfo.io', () => true, "jsonp").always((response) => {
                    const countryCode = (response && response.country) ? response.country : '';
                    callback(countryCode);
                });
            },
            utilsScript: "/bundles/intl-tel-input-17.0.0/build/js/utils.js", // just for formatting/placeholders etc
        });
        // populate prefix field with phone prefix
        const $phonePrefixInput = $('.phone-prefix-input');
        if ($phonePrefixInput.length === 1) {
            let fillPrefix = function(input, itiField) {
                input.val('+' + itiField.getSelectedCountryData().dialCode);
            }
            $(phoneInput).on('countrychange', function(e) {
                fillPrefix($phonePrefixInput, iti);
            });
            fillPrefix($phonePrefixInput, iti);
        }
    }
...
(function ($) {
    $('.alert-close').each(function (k, el) {
